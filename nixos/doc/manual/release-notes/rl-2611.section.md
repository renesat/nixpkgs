...
- `services.mautrix-telegram` has been adapted to support the new Go implementation of Mautrix-Telegram. Reconfiguration of the bridge is neccesary after switching to the new `mautrix-telegram-go` package. Support for the old Python bridge will be dropped from NixOS as it has not been maintained for a while. Refer to [the official release announcement](https://mau.fi/blog/2026-04-mautrix-release/).
